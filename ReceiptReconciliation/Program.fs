...
let ReadReceipt( filePath ) =
    ...
    if split.Length <> 3 then failwith "receipt name format invalid" //TODO we can get fails if we image multiple receipts in the same minute
...
let main argv = 
    ...
    #if DEBUG
    args <- [| @"/z/aolney/repos/receipt-checker/September2017_0403-tab.csv"; @"/z/aolney/repos/receipt-checker/Office Lens" |]
    #endif
